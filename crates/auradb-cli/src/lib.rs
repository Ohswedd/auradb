...
pub fn cmd_compatibility() -> String {
    ...
    format!(
        "AuraDB {ver}\n\
         ...
         Aura Connector (tested): 0.9.0\n\
         Aura Connector (supported): 0.5.x, 0.6.x, 0.7.x, 0.8.x, 0.9.x\n\
         Search features: bm25, hybrid, vector_exact, facets, aggregations, group_by, query_timeouts, ranked_pagination, query_profile, analyzers, snippets (vector_ann: opt-in HNSW preview with exact fallback; exact is the baseline)\n\
         Capabilities: {caps}\n\
...
mod tests {
    ...
    fn compatibility_reports_versions_and_capabilities() {
        ...
        // v1.5.0 pairs with Aura Connector v0.9.0 tested; v0.5.x/v0.6.x/v0.7.x/v0.8.x remain
        // supported for existing features. AWP 1, storage format v2 (both preserved).
        assert!(out.contains("Aura Connector (tested): 0.9.0"));
        assert!(out.contains("Aura Connector (supported): 0.5.x, 0.6.x, 0.7.x, 0.8.x, 0.9.x"));
