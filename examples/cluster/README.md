...
3. **Validate, then start:**
   ...
   ```bash
   # Local image (build it first: docker build -t auradb:0.9.2 .)
...
recommended connector for the current AuraDB v0.9.2 release, which leaves the wire
