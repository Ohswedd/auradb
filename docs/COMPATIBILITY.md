...
This document records what AuraDB v1.5.1 implements and how it interoperates with
the Aura Connector client library and the Aura Wire Protocol (AWP). AuraDB v1.5.1 is a
conformance-harness patch over v1.5.0 with no engine, wire, storage, index snapshot, or
connector behavior change; the implemented surface below is identical to v1.5.0. AuraDB
